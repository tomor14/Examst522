...
#' @usage density.estimate(x, d, h, method)
...
density.estimate <- function(x, d, h, method = "naive"){
  if(method == "naive"){
    # if h is not given:
    if(missing(h)){
      h <- (max(x)-min(x))/(1+log2(length(x)))    # Calculated by Sturges
    }
    # if d is not given:
    if(missing(d)){
    ...
    # if h is not given:
    if(missing(h)){
      h <- .9*sd(x)*length(x)^(-1/5)    # Calculated by Silverman
    }
    # if d is not given:
    if(missing(d)){
      px <- c(min(x), quantile(x, .25), quantile(x, .50), mean(x), quantile(x, .75), max(x))
